...
**Formato visual (mejorado 2026-07-21).** A pedido del usuario ("el formato es muy básico"), se agregó el estilo real del macro (analizado celda por celda con openpyxl, resolviendo el tema Office del archivo original a RGB exacto): relleno celeste (`#B8CCE4`) en título/cabecera/nombre-tipo de curso, relleno salmón (`#D99694`) en columnas de resumen, código de curso en rojo, link en azul, bordes medios en cabecera y finos en datos, anchos de columna por tipo de campo, alto de fila para el wrap del nombre de curso, y `freeze_panes` (cabecera + columnas fijas visibles al hacer scroll). No se encontró coloreado condicional real por nota en el original — sus reglas de formato condicional son restos sueltos sin lógica consistente, no se replicaron.

**Regla de negocio: "sin nota" = "NA" (no convocado), no un hueco de datos (corregido 2026-07-21).** Una comparación celda por celda contra `ejemplo formatos.xlsx` mostró que donde el original tiene "NA" tipeado a mano, el reporte generado inicialmente dejaba la celda en blanco, y un colaborador sin ninguna nota en absoluto (ANGAMARCA MORA HECTOR ROBERTO) no aparecía. El usuario aclaró que esto **no es un problema de datos faltantes**: la ausencia de una `Nota` para un colaborador+curso es la regla ya establecida de "no fue convocado a esa capacitación" — el equivalente exacto de `NO_APLICA`. Corregido: (1) una celda de curso sin ninguna nota (columna regular, no la compañera de supletorio) ahora muestra **"NA"** explícito; (2) el roster de `_datos_periodo()` ahora incluye a **todos los activos de la regional**, tengan o no alguna nota, unión de quienes ya no están activos pero sí tienen alguna nota REGULAR este año (para seguir mostrando su `SALIO`/`CAMBIO` real) — antes solo incluía a quien ya tenía alguna nota, dejando fuera a cualquier activo nunca convocado a nada.

> **Verificado con datos reales (2026-07-21):** generado para `anio=2026`, ENERO-FEBRERO, Quito, y comparado celda por celda contra `ejemplo formatos.xlsx` — 3 colaboradores (incluyendo el caso especial "A"=ASISTENCIA de CAGUA OVANDO KLEVER ANTONIO, que motivó el diseño del mapeo estado→código) coincidieron exacto, igual que las 5 fórmulas de resumen. Tras el fix de la regla NA: ANGAMARCA MORA HECTOR ROBERTO (1717138513) ya aparece con toda su fila en "NA", y ACERO QUILUMBAQUIN JOSE GABRIEL (1709896615) muestra "NA" en sus cursos sin nota en vez de celda vacía — verificado directamente contra el endpoint real.
...
- **Curso nuevo (Modo A):** el admin llena código, nombre, tipo (`PRESENCIAL`/`VIRTUAL`/`ZOOM`), mes, fechas, regional y **link de Moodle** (opcional, campo nuevo — 2026-07-21). Cada fila del CSV se guarda como `Nota(tipo="REGULAR", convocatoria=1)`.
  - **Fix (2026-07-21):** hasta ahora no existía forma de guardar `Curso.url_telcou` al crear un curso desde esta pantalla — quedaba siempre vacío (a diferencia de los cursos de la carga masiva 2026, que sí lo traían), lo que afectaba directamente al reporte "Calificaciones (formato macro)" (fila "link" de cada curso, sin dato). Recargar el mismo curso sin volver a escribir el link no lo borra (`COALESCE` en el upsert), igual que `convocatoria_habilitada`/`evaluado`.
